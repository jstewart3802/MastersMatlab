...
normalised_spectra=snv(amidenormMaster1);
cropped_spectra = normalised_spectra(1:end,1:200);
...
%WT = reshape(WTb, NumWT, 728); % have to put in the correct number
%TGb = amidenormMaster1(groupIndx == 0); 
%NumTG = numel(group2wtTG(group2wtTG == 0));
%TG = reshape(TGb, NumTG, 728); % have to put in the correct number

%% Sorts the normalised_spectra into WT or TG
groupIndxnorm = repmat(group2wtTGnorm, 1, length(cropped_spectra)); %logical index to the same size as the Spectra
NumWTnorm = numel(group2wtTGnorm(group2wtTGnorm == 1)); % gets how many WT 
WTbnorm = cropped_spectra(groupIndxnorm); 
WTnorm = reshape(WTbnorm, NumWTnorm, 200); % have to put in the correct number
TGbnorm = cropped_spectra(groupIndxnorm == 0); 
NumTGnorm = numel(group2wtTGnorm(group2wtTGnorm == 0));
TGnorm = reshape(TGbnorm, NumTGnorm, 200); % have to put in the correct number
